...
public: 	
    /*
	 Operación 'repatir' a implementar
	 Aparte de esta método, puedes implementar todas los métodos (privados) auxiliares
	 que consideres oportuno.

	 COMPLEJIDAD: Debes indicar aquí, justificadamente,la complejidad de
	 tu implementación:
	...
	*/
	...
	repartir 10 15 20 10 25  => 10 15 20 10 25 #25 10 20 15 10
	repartir 10 2 3 8 9  => 2 3 8 9 10 #10 9 8 3 2
	repartir 8 5  => 5 8 #8 5
	repartir 5 8  => 5 8 #8 5
	repartir 10  => 10 #10
	repartir  => #
	*/

	// RESUELTO EN 37' 50'' 43'''
    void repartir() {
		/***** A IMPLEMENTAR ****/
		Nodo* minumero = _prim;
		Nodo* izq = NULL;
		bool primermenor = false;
		if (longitud() > 1) 
		{
			Nodo* recorro = _prim->_sig;
			Nodo* aux = NULL;
			Nodo* aux2 = NULL;
			_ult = _prim;
			int i = 0;
			while (i < longitud() - 1) {
				// si es menor a mi numero
				if (recorro->_elem < minumero->_elem) 
				{
					if (recorro->_sig != NULL) {
						// guardo mi sig pos
						aux = recorro->_sig;
						// puente
						recorro->_ant->_sig = recorro->_sig;
						recorro->_sig->_ant = recorro->_ant;
					}
					// si es el primer menor
					if (!primermenor) {
						primermenor = true;
						// actualizo mi izq
						izq = recorro;
