	...
	void invertir(unsigned int i, unsigned int k) {
		...
		if (longitud() > 1) {
			...
			while (j != i) {
				...
				// avanzo
				recorro = recorro->_sig;
				++j;
			}

			// intercambio kx veces
			while ( kx > 0 ) {
				// intercambio
				aux = recorro->_sig;
				recorro->_sig = recorro->_ant;
				recorro->_ant = aux;
				// avanzo
				recorro = aux;
				kx--;
			...
			if (i > 1 && i + k < longitud() + 1) {
				// puente a la izq
				aux = anterior->_sig;
				anterior->_sig = recorro->_ant;
				recorro->_ant->_ant = anterior;				
				// puente a la dex
				aux->_sig = recorro;
				recorro->_ant = aux;				
			}
			else if(i > 1 && i + k >= longitud() + 1){
				// puente a la izq
				aux = anterior->_sig;
				recorro->_sig = recorro->_ant;
				anterior->_sig = recorro;
				recorro->_ant = anterior;
				// coloco ult
				_ult = aux;
				_ult->_sig = NULL;				
			}
