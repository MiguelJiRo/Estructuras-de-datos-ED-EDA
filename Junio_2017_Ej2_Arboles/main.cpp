...
*/
// RESUELTO EN 3' 37'' 98'''
int numero_singulares_aux(const Arbin<int>& a, int antepasados ,int &cuenta) {
	if (a.esVacio()) {
		return 0;
	}
	else {
		int izq = numero_singulares_aux(a.hijoIz(), antepasados + a.raiz(), cuenta);
		int der = numero_singulares_aux(a.hijoDer(), antepasados + a.raiz(), cuenta);

		if (izq + der == antepasados) {
			cuenta++;
		}
...
unsigned int numero_singulares(const Arbin<int>& a) {
	// A IMPLEMENTAR
	int cuenta = 0;
	numero_singulares_aux(a, 0, cuenta);
	return cuenta;
}
