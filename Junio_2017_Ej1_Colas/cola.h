...
void desplaza(unsigned int pos, unsigned int dist) {
	...
	if (dist > 0) 
	{
		...
		int i = 1;
		while (i != pos - 1) {
			// guardo pos anterior a la que se va a mover
			if (i == pos - distx - 1) {
				nuevapos = recorro;
			}
		...
		if (pos == longitud()) {
			...
			// hago puente en la pos antigua
			recorro->_sig = aux->_sig;
		}
		// coloco en la nueva pos
		if (pos - distx == 1) {
			// estoy en el _prim
			aux->_sig = _prim;
			_prim = aux;
			...
			// en otra posicion delante de prim
			recorro = nuevapos->_sig;
			nuevapos->_sig = aux;
			aux->_sig = recorro;
		}
	}
