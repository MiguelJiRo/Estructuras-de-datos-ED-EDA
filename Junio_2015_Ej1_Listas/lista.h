...
public: 	
	...
	*/
	...
	/*
	2. (2 puntos) Implementa una función (externa al TAD List) que dada una lista enlazada de ca- 
   racteres (List<char>) 1, y dos números enteros i y k, invierta el segmento que comienza 
   en la posición i-ésima de la lista 1 (se empieza desde la posición 1) y contiene k elementos. 
   Por ejemplo, si la lista está formada por los elementos a b c d e f, e invertimos el segmento 
   que comienza en la posición 3 y tiene longitud 3, entonces la lista resultante a b e d c f.Si 
   i + k > l.size() + 1 entonces se invertirá el segmento que va desde la posición i-ésima hasta el   
   final de la lista. Se valorará la complejidad en tiempo y en espacio de la función implementada,   
	...
	*/
	void invertir(unsigned int i, unsigned int k) {
		// IMPLEMENTAR
		Nodo *posInic = _prim;
		Nodo *aux = NULL;
		...
		while (kfinal > 0) 
		{
			if (i +  kfinal - 1 == j) 
			{
				// guardo el ultimo
				ultimo = posInic;
				// guardo la posicion posterior
				posterior = posInic->_sig;
				//std::cout << "posteior => " << posInic->_sig->_elem << "\n";

				while (kfinal > 0) {
					//std::cout << "#2 Estoy en => " << posInic->_elem << "\n";
					// intercambio
					aux = posInic->_sig;
					posInic->_sig = posInic->_ant;
					posInic->_ant = aux;
					// avanzo
					if(kfinal > 1)
						posInic = posInic->_sig;
					kfinal--;
				}
				// coloco final
				posInic->_sig = posterior;
				posterior->_ant = posInic;
				// coloco principio
				anterior->_sig = ultimo;
				ultimo->_ant = anterior;
			}
			//std::cout << "#1 Estoy en => " << posInic->_elem << "\n";
			// guardo la posicion anterior;
			if (j == i - 1) {
				//std::cout << "anterior => " << posInic->_elem << "\n";
				anterior = posInic;
			}
			if (kfinal > 0) 
			{	// avanzo
