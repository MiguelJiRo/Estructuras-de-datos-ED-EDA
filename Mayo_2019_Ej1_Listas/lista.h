...
public:
	...
	*/
	...
	// Resuelto en 16' 53'' 74'''
    void estremece() {
		  /* A IMPLEMENTAR */
		  
		Nodo* por_el_ppio = _prim;
		Nodo* por_el_final = NULL;
		Nodo* recorro = _prim;
		Nodo* aux = NULL;
		...
		if (longitud() > 2) {
			...
			while (i < longitud()) {
				...
				{					
					// no es primer par
					if (i != 0) {
						// hago puente a la izq
						por_el_ppio->_sig = recorro;
						recorro->_ant = por_el_ppio;
						por_el_ppio = por_el_ppio->_sig;
					}
				}
				// encuentro pos impar
				else 
				{
					...
					if (i == 1) {
						_ult = recorro;
						por_el_final = recorro;
						recorro->_sig = NULL;
					}
					else {
						// hago puente a la dex
						por_el_final->_ant = recorro;
						recorro->_sig = por_el_final;
						por_el_final = por_el_final->_ant;
					}
				}
				// avanzo recorro
				recorro = aux;
			...
			// union final
			por_el_final->_ant = por_el_ppio;
			por_el_ppio->_sig = por_el_final;
		}		  
 	}
