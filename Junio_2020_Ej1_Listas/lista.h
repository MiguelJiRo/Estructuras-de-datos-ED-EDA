...
public:
	...
	*/
	void transponer() {
		...
		if (_prim->_elem != minimo) 
		{
			if (_prim != anterior) {
				anterior->_ant->_sig = elegido;
				if(elegido != _ult)
					elegido->_sig->_ant = anterior;
			}
			else {
				elegido->_sig->_ant = anterior;
				_prim = elegido;
			...
			aux = elegido->_sig;
			elegido->_sig = anterior;
			elegido->_ant = anterior->_ant;

			anterior->_sig = aux;
			anterior->_ant = elegido;
